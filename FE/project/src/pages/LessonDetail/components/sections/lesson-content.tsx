...
const convertToEmbedUrl = (url?: string | null) => {
  ...
  try {
    const u = new URL(url);
    const videoId = u.searchParams.get("v") || url.split("youtu.be/")[1]?.split("?")[0];
...
const LessonContent = ({ lesson }: LessonContentProps) => {
  ...
  const [isAllowedToComplete, setIsAllowedToComplete] = useState(lesson.isDone || false);
    ...
    const interval = setInterval(() => {
      ...
      if (watchTimeRef.current >= 10 && !isAllowedToComplete) {
        ...
        if (!unlockToastRef.current) {
          unlockToastRef.current = true;   // <– KHÔNG BAO GIỜ LẶP LẠI
          toast({
  ...
  const markLessonCompleted = async () => {
    try {
      ...
      const res = await api.post(`/student/lessons/${lesson.id}/progress`, {
        isDone: true,
        ...
        return (
            <div className="absolute inset-0 flex flex-col items-center justify-center px-10 py-12 bg-gradient-to-br from-blue-50 to-purple-50 text-center">
              ...
              <p className="text-gray-500 max-w-2xl mt-6">
                When you're done reading, click the button below.
              </p>
            </div>
        );
  ...
  return (
      <div className="lg:col-span-2">
        ...
        {/* ⭐ BUTTON HOÀN THÀNH – Đặt dưới video */}
        <div className="flex justify-end mb-8">
          <Button
              disabled={!isAllowedToComplete || isUpdating || lesson.isDone}
            ...
            <Tabs defaultValue="materials" className="w-full">
              ...
              <TabsContent value="transcript" className="p-6">
                ...
                <pre className="whitespace-pre-wrap text-gray-800 leading-relaxed font-sans">
                  {lesson.content || lesson.transcript || "No content available."}
                </pre>
                </Card>
              </TabsContent>
            </Tabs>
